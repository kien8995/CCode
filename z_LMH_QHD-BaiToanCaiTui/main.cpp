...
void optimize()	//tim bang phuong an bang cong thuc truy hoi
{
	set(F[0], 0);	//dien co so quy hoach dong
	forle(i,1,n)
	{
		forle(j,0,m)
		{
			...
			//sau do danh gia, neu chon goi i duoc loi hon thi dat lai F[i][j]
...
void trace()	//truy vet tim nghiem toi uu
{
	PRI(F[n][m]);
	while (n != 0)
	{
		if (F[n][m] != F[n - 1][m])	//neu co chon goi thu n
		{
			printf("%d ", n);
			m -= W[n];//da chon goi thu n roi thi chi duoc mang them trong luong m-W[n] nua thoi
		}
		--n;
	}
	printf("\n");

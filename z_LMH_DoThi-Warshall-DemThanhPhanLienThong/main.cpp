...
//-------------------------------------------------
const int MAX = 100;
bool a[MAX + 1][MAX + 1];	//ma tran ke cua do thi
bool Free[MAX + 1];	//Free[v]==true	<-> v chua duoc liet ke vao thanh phan lien thong nao
int n, m;
void enter()
{
	int u, v;
	...
	forle(v,1,n)
	{
		a[v][v] = true;		//di nhien tu v co duong di den chinh v
	}
	forle(i,1,m)
	{
		RII(u, v);
...
int main()
{
	...
	int count = 0;
	set(Free, true);// moi dinh chua duoc liet ke vao thanh phan lien thong nao
	forle(u,1,n)
	{
		if (Free[u])//neu dinh u chua duoc liet ke vao thanh phan lien thong nao
		{
			++count;
			printf("Conected component %d:\n", count);
			forle(v,1,n)
			{
				if (a[u][v])	//xet nhung dinh v ke voi u ( tren bao dong )
				{
					printf("%d, ", v);//liet ke nhung dinh do vao thanh phan lien thong cua u
					Free[v] = false;	//liet ke dinh nao danh dau dinh do
				}
			}
			printf("\n");
		}

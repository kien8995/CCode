...
//-------------------------------------------------
const int MAX = 100;
bool a[MAX + 1][MAX + 1]; //ma tran ke cua do thi, a[u][v] = true <-> (u,v) la canh
bool Free[MAX + 1];	//mang danh dau Free[v] = true neu chua di qua dinh v
int x[MAX + 1];	//chu trinh hamilton se tim la 1=x[1]->x[2]->...->x[n]->x[1]=1
int n, m;
void enter()
{
	RII(n, m);
...
void printResult()	//in ket qua neu tim thay chu trinh hamilton
{
	forle(i,1,n)
	{
		printf("%d ", x[i]);
...
void attempt(int i)	//thu cac cach chon dinh thu i trong hanh trinh
{
	forle(j,1,n)	//dinh thu i(x[i] co the chon trong cac dinh)
	{
		if (Free[j] && a[x[i - 1]][j])	//ke voi x[i-1] va chua bi di qua
		{
			x[i] = j;	//thu 1 cach chon x[i]
			if (i < n)	//neu chua chon den x[n]
			{
				Free[j] = false;	//danh dau j da di qua
				attempt(i + 1);	//de cac lan sau khong chon j nua
				Free[j] = true;	//se thu phuong an khac cho x[i] nen se bo danh dau
			}
			else	//neu da chon den x[n]
			{
				if (a[j][x[1]])	//va neu x[n] lai ke voi x[1] thi ta co chu trinh hamilton
				{
					printResult();
				}
			}
...
#endif
	enter();
	set(Free, true);	//khoi tao cac dinh deu chua di qua
	x[1] = 1;
	Free[1] = false;	//bat dau tu dinh 1
	attempt(2);	//thu cac cach chon dinh lien tiep
#ifdef LOCAL_DEFINE

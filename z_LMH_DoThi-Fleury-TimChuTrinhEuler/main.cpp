...
//ham kiem tra, neu xoa di 1 canh noi (x,y) thi y con quay lai duoc x hay khong
bool canGoBack(int x, int y)
{
	int que[MAX + 1];	//hang doi dung cho bfs
	int front, rear;	//chi so dau va cuoi cua hang doi
	bool Free[MAX + 1];	//mang danh dau
	//thu xoa 1 canh noi (x,y) <-> so canh noi (x,y) giam di 1
	--a[x][y];
	--a[y][x];
	set(Free, true);//sau do ap dung bfs de kiem tra xem tu y co quay lai duoc x hay khong
	Free[y] = false;
	front = rear = 1;
	que[1] = y;
	while (front <= rear)
	...
	//o tren da thu xoa canh gio phai phuc hoi
	++a[x][y];
	++a[y][x];
	return !Free[x];
}
void findEulerCircuit()	//thuat toan Fleury
{
	int current, next, Count;
	current = 1;
	printf("%d ", current);	//bat dau tu dinh current == 1
	Count = 1;
	while (next != 0)	//cho toi khi khong di tiep duoc nua
	{
		next = 0;
		forle(v,1,n)
		{
		...
		{
			--a[current][next];
			--a[next][current];	//xoa bo canh vua di qua
			printf("%d ", next);	//in ket qua di toi next
			++Count;
			if (Count % 16 == 0)		//in toi da 16 dinh tren 1 dong
			{
				printf("\n");
			}
			current = next;	//lap lai tiep tuc voi dinh dang dung la next
		}
	}
}
/**MAIN FUNCTION**/

...
bool findPath()	//tim duong tang luong tren Gf
{
	...
	front = rear = 1;
	que[1] = s;
	trace[s] = n + 1;	//trace[v] ==0 <-> chua tham
	int u;
	while (front <= rear)
	{
		u = que[front];	//lay u khoi queue
		++front;
		forle(v,1,n)
		{
			if (trace[v] == 0 && c[u][v] > f[u][v])	//xet moi v chua tham ke u tren Gf
			{
				trace[v] = u;
				if (v == t)	//den duoc t thi dung thuat toan
				{
					return true;
				}
				++rear;
				que[rear] = v;	//day v vao queue
			}
		}
	}
	return false;
}
void incFlow()	//tang luong doc duong tang luong f= f+fp
{
	int delta, u, v;
	delta = INF;
	v = t;
...
void printResult()
{
	...
	forle(u,1,n)
	{
		forle(v,1,n)
		{
			if (f[u][v] > 0)	//chi quan tam den nhung cung co luong duong
			{
				printf("f(%d, %d) = %d\n", u, v, f[u][v]);
				if (u == s)
				{

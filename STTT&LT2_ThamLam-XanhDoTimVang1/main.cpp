...
/**DEBUG**/
#define debug1(x) do { std::cerr << #x << ": " << x << std::endl; } while (0)
...
int main()
{
	...
	FORL(i,0,4)
	{
		RII(cl[i].quantity, cl[i].length);
	}
	nx = lcm<int>(cl[0].length, cl[2].length) / cl[0].length;
	nt = lcm<int>(cl[0].length, cl[2].length) / cl[2].length;
	nd = lcm<int>(cl[1].length, cl[3].length) / cl[1].length;
	nv = lcm<int>(cl[1].length, cl[3].length) / cl[3].length;
	xt = min(cl[0].quantity / nx, cl[2].quantity / nt);
	dv = min(cl[1].quantity / nd, cl[3].quantity / nv);
	//debug2("dddd", nx, nt, nd, nv);
	area = (nx * xt * cl[0].length) * (nd * dv * cl[1].length);
	PRI(area);
	printf("%d %d %d %d\n", nx * xt, nd * dv, nt * xt, nv * dv);
#ifdef LOCAL_DEFINE

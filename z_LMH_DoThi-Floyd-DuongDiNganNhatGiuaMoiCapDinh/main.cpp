...
int c[MAX + 1][MAX + 1];
int trace[MAX + 1][MAX + 1]; //trace[u][v] dinh lien sau u, tren duong di tu u toi v
int n, m, s, f;
...
void floyd()
{
	forle(u,1,n)
	{
		forle(v,1,n)
		{
			trace[u][v] = v;//gia su duong di ngan nhat giua moi cap dinh la duong truc tiep
		}
	}
	//thuat toan floyd
	forle(k,1,n)
	{
		forle(u,1,n)
		{
			forle(v,1,n)
			{
				if (c[u][v] > c[u][k] + c[k][v])	//neu di qua k tot hon
				{
					c[u][v] = c[u][k] + c[k][v];//ghi nhan duong moi thay cho duong cu
					trace[u][v] = trace[u][k];
				}
			}
		}

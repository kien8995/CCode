...
{
	int jmax;
	a[0] = -INF;	//Them 2 phan tu canh 2 dau day a
	a[n + 1] = INF;
	L[n + 1] = 1;		//dien co so QHD vao bang phuong an
	forge(i,n,0)	//tinh bang phuong an
	{
		//chon trong cac chi so j dung sau i thoa man a[j] > a[i] ra chi so jmax, co L[jmax] lon nhat
		jmax = n + 1;
		forle(j,i+1,n+1)
		{
		...
		L[i] = L[jmax] + 1;	//luu do dai day con tang dai nhat bat dau tai a[i]
		T[i] = jmax;// luu vet phan tu dung lien sau a[i] trong day con tang dai nhat do la a[jmax]
	}
}
void result()
{
	PRI(L[0] - 2);	//chieu dai day con tang dai nhat

...
void bfs()	// tim kiem theo chieu rong
{
	int que[MAX];
	int front, rear, u;
	front = rear = 1; 	// chi so dau va chi so cuoi hang doi
	que[1] = s;		//khoi tao hang doi ban dau chi gom 1 dinh s
	set(trace, 0);	//cac dinh deu chua danh dau
	trace[s] = -1;	//ngoai tru dinh s
	while (front <= rear)	//lap cho den khi hang doi rong
	{
		u = que[front];		//lay tu hang doi ra 1 dinh u
		++front;
		forle(v,1,n)
		{
			if (a[u][v] && trace[v] == 0)//xet nhung dinh v chua tham ke voi u
			{
				++rear;
				que[rear] = v;	//day v vao hang doi
				trace[v] = u;	//luu vet duong di, cung la danh dau luon
			}
		}
	}
}

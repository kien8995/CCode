...
const int MAX = 100;
bool a[MAX + 1][MAX + 1];
int number[MAX + 1], low[MAX + 1], parent[MAX + 1];
int n, m, Count;
void enter()
{
	RII(n, m);
...
void visit(int u)	//duyet dfs, dinh chieu, danh so
{
	++Count;
	number[u] = Count;	//truoc het danh so u
	low[u] = n + 1;	//khoi gan low[u] = oo
	forle(v,1,n)
	{
		if (a[u][v])	//xet moi v ke u
		{
			a[v][u] = false;	//bo di cung (v,u)
			if (parent[v] == 0)	//parent[v] == 0 <-> v chua tham, (u,v) la cung dfs
			{
				parent[v] = u;
				visit(v);	//tham v
				if (low[u] > low[v])
				{
					low[u] = low[v];	//cuc tieu hoa low[u] theo low[v]
				}
			}
			else	//v da tham, (u,v) la cung nguoc
			{
				if (low[u] > number[v])
				{
					low[u] = number[v];	//cuc tieu low[u] theo number[v]
				}
			}
		}
	}
...
{
	Count = 0;
	set(parent, 0);	//danh dau moi diem deu chua tham
	forle(u,1,n)
	{
		if (parent[u] == 0)	//gap 1 dinh chua tham
		{
			parent[u] = -1;	//cho u la goc cay dfs
...
void result()
{
	int nChildren[MAX + 1];
	bool isCut[MAX + 1];
	set(nChildren, 0);	//nChildren[u] la so nhanh con cua nhanh dfs goc u
	forle(v,1,n)
	{
		if (parent[v] != -1)
		{
	...
	{
		int u = parent[v];
		if (u != -1 && low[v] >= number[v])	//(u,v) la cau
		{
			printf("(%d, %d)\n", u, v);
		}
	}
	...
	forle(v,1,n)
	{
		...
		{
			int u = parent[v];
			//neu low[v] >= number[u] khop <-> u khong phai la goc cay dfs hoac u co >= 2 nhanh con
			if (low[v] >= number[u])
			{
				isCut[u] |= ((parent[u] != -1) | (nChildren[u] >= 2));
			}
		}
	}
	forle(u,1,n)

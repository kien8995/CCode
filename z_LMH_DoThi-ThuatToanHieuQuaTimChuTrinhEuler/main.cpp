...
//-------------------------------------------------
const int MAX = 100;
const int MAXE = 20000;	//so canh toi da
int a[MAX + 1][MAX + 1];
int Stack[MAXE + 1];
int n, top;
void enter()
...
void push(int v)	//day 1 dinh vao ngan xep
{
	++top;
	Stack[top] = v;
}
...
int get()	//tra ve phan tu o dinh top ngan xep
{
	return Stack[top];
}
void findEulerCircuit()
{
	...
	top = 1;
	Count = 0;
	while (top != 0)	//chung nao ngan xep chua rong
	{
		int u = get();	//u la phan tu o dinh ngan xep
		forle(v,1,n)
		{
			if (a[u][v] > 0)	//xet cac canh lien thuoc voi u , neu thay
			{
				//xoa canh do khoi do thi
				--a[u][v];
				--a[v][u];
		...
		if (u == get())	//neu phan tu o dinh ngan xep van la u	-> vong lap tren khong tim thay dinh nao ke voi u
		{
			++Count;
			printf("%d ", pop());	//in ra phan tu o dinh ngan xep
		}
	}
}
/**MAIN FUNCTION**/

...
int d[MAX + 1];
int trace[MAX + 1];
bool Free[MAX + 1];	//Free[u] = true <-> u co nhan tu do
int n, m, s, f;
void enter()	//trong so cac cung phai la khong am
{
...
void init()	//khoi tao cac nhan d[v], cac dinh deu duoc coi la tu do
{
	forle(i,1,n)
	{
		d[i] = INF;
...
void dijkstra()
{
	...
	while (true)
	{
		...
		//thuat toan da cho se dung khi cac dinh tu do deu co nhan +oo hoac da chon den dinh f
		if (u == 0 || u == f)
		{
			break;
		}
		//co dinh nhan dinh u
		Free[u] = false;
		// dung dinh u toi uu nhung dinh tu do ke voi u
		forle(v,1,n)
		{

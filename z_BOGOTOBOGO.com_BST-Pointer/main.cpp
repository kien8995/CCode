...
Tree* lowestCommonAncestor(Tree *node, Tree *p, Tree *q)
{
	...
	if (node->left == p || node->left == q || node->right == p
			|| node->right == q)
	{
		return node;
	}
...
void level_even_odd(struct Tree *node)
{
	...
	for (int i = 0; i < (int) evenVec.size(); i++)
	{
		cout << evenVec[i] << " ";
	}
	cout << endl << "odd level elements : ";
	for (int i = 0; i < (int) oddVec.size(); i++)
	{
		cout << oddVec[i] << " ";
	}
	cout << endl;
...
/* In Order Predecessor - a node which has the next lower key */
char predecessorInOrder2(struct Tree *node)
{
	/* if the node has left child, predecessor is Tree-Maximum */
	if (node->left != NULL)
	{
		return maxTree(node->left)->data;
	}
	Tree *y = node->parent;
	/* if it does not have a left child,
	 predecessor is its first left ancestor */
...
void deleteKey(Tree *root, char ch)
{
	...
	{
		char ch_pred = predecessorInOrder2(node);
		pred = lookUp(root, ch_pred);
...
int main(int argc, char **argv)
{
	...
	if (ancestor)
	{
		cout << "The lowest common ancestor of " << ch1 << " and " << ch2
	...
	for (int m = 0; m < (int) levVec.size(); m++)
	{
		cout << "Level at " << m << ": ";
		for (int n = 0; n < (int) levVec[m].size(); n++)
